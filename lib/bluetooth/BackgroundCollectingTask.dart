      ...
      while (true) {
        ...
        if (index >= 0 && _buffer.length - index >= 7) {
          final DataSample sample = DataSample(
              humidity1: (_buffer[index + 1] + _buffer[index + 2] / 100),
              humidity2: (_buffer[index + 3] + _buffer[index + 4] / 100),
              battery: (_buffer[index + 5] + _buffer[index + 6] / 100),
          ...
          //print("${sample.timestamp.toString()} -> ${sample.humidity1} / ${sample.humidity2}");
        }
        // Otherwise break
        else {
          break;
